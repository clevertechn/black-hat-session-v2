...
async function deleteSession(id) {
    ...
    if (storageBackend === 'mongodb') {
        const res = await mongoModel.findOneAndDelete({ shortId: safeId });
        return !!res;
    } else if (storageBackend === 'postgresql') {
        const result = await pgPool.query(
            'DELETE FROM gifted_sessions WHERE short_id = $1',
            [safeId]
        );
...
async function updateSessionActivity(id, isOnline = true) {
    const safeId = id.replace(/[^a-zA-Z0-9]/g, '');
...
module.exports = { init, isConfigured, saveSession, getSession, updateSessionActivity, deleteSession };
